...
static void init_golden_trace() {
    golden_trace_fp = fopen("golden.trace", "w");
    Assert(golden_trace_fp, "Cannot open golden.trace");
}
...
static void load_entry() {
    ...
    fseek(fp, 0, SEEK_SET);
    ret = fread((void *) (hw_mem + (ENTRY_START & 0x1fffffff)), file_size, 1,
                fp);  // load .text segment to memory address 0x1fc00000
    assert(ret == 1);
