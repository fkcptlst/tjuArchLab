...
/*------------------- ?????? -------------------*/
`define RST_ENABLE      1'b0                // ??¦Ë?????§¹  RST_ENABLE
`define RST_DISABLE     1'b1                // ??¦Ë?????§¹
`define ZERO_WORD       32'h00000000        // 32¦Ë?????0
`define ZERO_DWORD      64'b0               // 64¦Ë?????0
`define WRITE_ENABLE    1'b1                // ???§Õ
`define WRITE_DISABLE   1'b0                // ???§Õ
`define READ_ENABLE     1'b1                // ????
`define READ_DISABLE    1'b0                // ?????
`define ALUOP_BUS       7 : 0               // ?????¦Å????aluop_o?????
`define SHIFT_ENABLE    1'b1                // ??¦Ë?????? 
`define ALUTYPE_BUS     2 : 0               // ?????¦Å????alutype_o?????  
`define TRUE_V          1'b1                // ???"??"  
`define FALSE_V         1'b0                // ???"??"  
`define CHIP_ENABLE     1'b1                // §à????  
`define CHIP_DISABLE    1'b0                // §à????  
`define WORD_BUS        31: 0               // 32¦Ë??
`define DOUBLE_REG_BUS  63: 0               // ????????¨¹???????????????
`define RT_ENABLE       1'b1                // rt??????
`define SIGNED_EXT      1'b1                // ??????????
`define IMM_ENABLE      1'b1                // ????????????
`define UPPER_ENABLE    1'b1                // ????????¦Ë???
`define MREG_ENABLE     1'b1                // §Õ???¦Ä›¥???????????
`define BSEL_BUS        3 : 0               // ????›¥??????????????
...
`define EXC_CODE_BUS    4:0              //?§Ø????????
`define EXC_INT         5'b00000         //?§Ø???
`define EXC_ADEL        5'b00100         //????????????§µ??????????
`define EXC_ADES        5'b00101         //?›¥?????§µ??????????
